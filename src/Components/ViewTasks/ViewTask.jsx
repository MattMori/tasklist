...
const ViewTasks = ({ tasks, DeleteTask, toggleCompleted, EditTask }) => {
  ...
  const getIsDoneIcon = (completed) => {
    if (completed) {
      return Completed; // Ícone para tarefa concluída (completed.svg)
    } else {
      return uncompleted; // Ícone para tarefa não concluída (uncompleted.svg)
    }
  };
  ...
  return (
    <>
      {tasks.length === 0 && "Sem tarefas" /* ShortCircuit */}
